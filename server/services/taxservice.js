...
//Repository
const TaxRepository = require('../repository/taxrepository.js');
const _taxRepoInstance = new TaxRepository();
...
class TaxService {
    ...
    async list(pParam){
        ...
        var xResultList = await _taxRepoInstance.list(pParam);
        ...
        if( xResultList.count > 0 ){
            ...
            for( var index in xRows ){
                xJoArrData.push({
                    ...
                    type_name: xRows[index].type == 1 ? 'Include' : xRows[index].type == 2 ? 'Exclude' : '',
    ...
    async save(pParam){
        ...
        if( xAct == "add" ){           
            ...
            if( xFlagProcess ){
                var xAddResult = await _taxRepoInstance.save( pParam, xAct );
    ...
    async delete( pParam ){
        ...
        if( xFlagProcess ){
            ...
            var xDeleteResult = await _taxRepoInstance.delete( pParam );
