					...
					status: {
						...
						name:
							xRows[index].status == 1
								? 'Active'
								: xRows[index].status == 0 ? 'Inactive' : xRows[index].status == -1 ? 'Cancel' : ''
					},
					status_approval: {
						id: xRows[index].status_approval,
						name:
							xRows[index].status_approval == 1
								? 'To Approve'
								: xRows[index].status_approval == 0
									? 'Draft'
									: xRows[index].status_approval == 2 ? 'Approved' : 'Reject'
					},
				...
				xJoData = {
					...
					status: {
						...
						name:
							xResult.status == 1
								? 'Active'
								: xResult.status == 0 ? 'Inactive' : xResult.status == -1 ? 'Cancel' : ''
					},
					status_approval: {
						id: xResult.status_approval,
						name:
							xResult.status_approval == 1
								? 'To Approve'
								: xResult.status_approval == 0
									? 'Draft'
									: xResult.status_approval == 2 ? 'Approved' : 'Rejected'
					},
		...
		if (xFlagProcess) {
			if (xAct == 'add') {
				...
				if (xAddResult.created_id != null && xAddResult.created_id != '') {
					...
					var xProcNo = await _globalUtilInstance.generateProcurementNo(
						xAddResult.clear_id,
						pParam.company_code
					);
					var xParamUpdate = {
	...
	async cancel(pParam) {
		...
		if (xFlagProcess) {
			...
			if (xUpdateResult.status_code == '00') {
				...
				var xResultFetchMatrix = await _oAuthService.addApprovalMatrix(
					pParam.method,
					pParam.token,
					xParamFetchMatrix
				);
