...
class ProcurementVendorService {
		...
		if (xFlagProcess) {
			...
			if (xResult != null) {
				xJoData = {
					...
					procurement_id: await _utilInstance.encrypt(
						xResult.procurement_id.toString(),
	...
	async vendorConfirm(pParam) {
		...
		if (xFlagProcess) {
			...
			var xAddQuotationItem = await _procurementQuotationItemServiceInstance.addItemCopyFromProcurementItem(
				pParam
			);
