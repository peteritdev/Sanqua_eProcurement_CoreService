...
module.exports = {
	...
	purchaseRequestProject_EstimateNotif,
	...
	// purchaseRequestDetail_Subtitute,
	purchaseRequestDetail_SaveLink,
	purchaseRequestDetail_UpdateCAManual
};
...
async function purchaseRequestProject_EstimateNotif(req, res) {
	var joResult;
	var oAuthResult = await _oAuthServiceInstance.verifyToken(req.headers['x-token'], req.headers['x-method']);
	...
	if (oAuthResult.status_code == '00') {
		if (oAuthResult.token_data.status_code == '00') {
			...
			if (errors.length != 0) {
				...
				req.query.method = req.headers['x-method'];
				req.query.token = req.headers['x-token'];
				joResult = await _serviceInstance.fpbProjectEstimateNotif(req.query);
				joResult = JSON.stringify(joResult);
			}
		} else {
			joResult = JSON.stringify(oAuthResult);
...
async function purchaseRequest_TransactionHistory(req, res) {
	var joResult;
	var oAuthResult = await _oAuthServiceInstance.verifyToken(req.headers['x-token'], req.headers['x-method']);
	...
	if (oAuthResult.status_code == '00') {
		if (oAuthResult.token_data.status_code == '00') {
			...
			if (errors.length != 0) {
				...
				req.query.method = req.headers['x-method'];
				req.query.token = req.headers['x-token'];
				joResult = await _serviceInstance.transaction_history(req.query);
				joResult = JSON.stringify(joResult);
			}
		} else {
			joResult = JSON.stringify(oAuthResult);
...
async function purchaseRequestDetail_UpdateFulfillment(req, res) {
	var joResult;
	var oAuthResult = await _oAuthServiceInstance.verifyToken(req.headers['x-token'], req.headers['x-method']);
...
async function purchaseRequest_FetchMatrix(req, res) {
	var joResult;
	var oAuthResult = await _oAuthServiceInstance.verifyToken(req.headers['x-token'], req.headers['x-method']);
	...
	if (oAuthResult.status_code == '00') {
		if (oAuthResult.token_data.status_code == '00') {
			...
			if (errors.length != 0 && req.body.act == 'add') {
				...
				req.body.notification_via_wa = oAuthResult.token_data.result_verify.notification_via_wa;
				req.body.notification_via_telegram = oAuthResult.token_data.result_verify.notification_via_telegram;
				joResult = await _serviceInstance.fetchMatrixFPB(req.body);
				joResult = JSON.stringify(joResult);
			}
		} else {
			joResult = JSON.stringify(oAuthResult);
...
async function purchaseRequestDetail_CancelItem(req, res) {
	var joResult;
	var oAuthResult = await _oAuthServiceInstance.verifyToken(req.headers['x-token'], req.headers['x-method']);
...
async function purchaseRequestDetail_SaveLink(req, res) {
	var joResult;
	var oAuthResult = await _oAuthServiceInstance.verifyToken(req.headers['x-token'], req.headers['x-method']);
	...
	if (oAuthResult.status_code == '00') {
		if (oAuthResult.token_data.status_code == '00') {
			...
			if (errors.length != 0 && req.body.act == 'add') {
				...
				req.body.token = req.headers['x-token'];
				req.body.method = req.headers['x-method'];
				joResult = await _serviceDetailInstance.save_link(req.body);
				joResult = JSON.stringify(joResult);
			}
		} else {
			joResult = JSON.stringify(oAuthResult);
...
// async function purchaseRequestDetail_DeviationList(req, res) {
// 	var joResult;
// 	var oAuthResult = await _oAuthServiceInstance.verifyToken(req.headers['x-token'], req.headers['x-method']);
...
// 				req.query.method = req.headers['x-method'];
// 				req.query.token = req.headers['x-token'];
// 				joResult = await _serviceDetailInstance.deviationItemList(req.query);
// 				joResult = JSON.stringify(joResult);
// 			}
// 		} else {
// 			joResult = JSON.stringify(oAuthResult);
...
// async function purchaseRequestDetail_Subtitute(req, res) {
// 	var joResult;
// 	var oAuthResult = await _oAuthServiceInstance.verifyToken(req.headers['x-token'], req.headers['x-method']);
...
// 				req.body.token = req.headers['x-token'];
// 				req.body.method = req.headers['x-method'];
// 				joResult = await _serviceDetailInstance.subtitute(req.body);
// 				joResult = JSON.stringify(joResult);
// 			}
// 		} else {
// 			joResult = JSON.stringify(oAuthResult);
...
async function purchaseRequestDetail_UpdateCAManual(req, res) {
	var joResult;
	var oAuthResult = await _oAuthServiceInstance.verifyToken(req.headers['x-token'], req.headers['x-method']);
	...
	if (oAuthResult.status_code == '00') {
		if (oAuthResult.token_data.status_code == '00') {
			...
			if (errors.length != 0 && req.body.act == 'add') {
				...
				req.body.token = req.headers['x-token'];
				req.body.method = req.headers['x-method'];
				joResult = await _serviceDetailInstance.updateCA(req.body);
				joResult = JSON.stringify(joResult);
			}
		} else {
			joResult = JSON.stringify(oAuthResult);
